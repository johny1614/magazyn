import os.path as path, sys
sys.path.append("..")
from params.params import FlowParams
from utils.utils import q,positive
from timeService import TimeService as ts
class Road:
    def __init__(self, name, time, cell_index_start, flow_input_type,flow_output_type):
        self.time=time
        self.stagerredGrid = StageredGrid(cell_index_start)
        self.normalGrid = NormalGrid(cell_index_start)
        self.name = name
        self.turn_probability = {'right':0.3,'straight':0.7}
        self.flow_input_type=flow_input_type
        self.flow_output_type=flow_output_type
        self.light=0
        self.input_flow = [[0.6]*4]*len(ts.time_range)
class StageredGrid:
    def __init__(self,cell_index_start):
        numberOfCells=int(FlowParams.x_max/FlowParams.d_x/2+2)
        self.cells=[Cell(cell_no) for cell_no in range(cell_index_start,cell_index_start+numberOfCells)]
    def print_p(self):
        print([cell.p for cell in self.cells])
class NormalGrid:
    def __init__(self,cell_index_start):
        numberOfCells=int(FlowParams.x_max/FlowParams.d_x/2+1)
        self.cells=[Cell(cell_no) for cell_no in range(cell_index_start,cell_index_start+numberOfCells)]
    def print_p(self):
        print([cell.p for cell in self.cells])
class Cell(object):
    def __init__(self,number):
        self.number=number
        self._p=FlowParams.p_init
        self._q=FlowParams.q_init if FlowParams.q_init else q(FlowParams.p_init)
    @property
    def p(self):
        return self._p
    @p.setter
    def p(self, value):
        self._p = min([positive(value),FlowParams.p_max])
        self.q = positive(q(self._p))
    @property
    def q(self):
        return self._q
    @q.setter
    def q(self, value):
        self._q = positive(value)
