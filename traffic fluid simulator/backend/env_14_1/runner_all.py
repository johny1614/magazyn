        ...
        for den1 in range(25):
            for den2 in range(25):
                for den3 in range(25):
                    to_predict.append([den0, den1, den2, den3, 0])  # na razie dla fazy 0
...
for run in runs:
    ...
    timeToLearn = 500000
    startTime = timer()
    generate_random_epochs(learntAgents=False, epochs=range(Globals().vp().first_epochs_range))  # bierze nowych agentow i tu jest 'is'
