...
class Testing(unittest.TestCase):
    ...
    #     env.agents[0].save_batch()
    #     env.agents[0].reshape_rewards()
    #     self.assertAlmostEqual(env.agents[0].memories[60].reward, 9.1, 0)
    #     self.assertAlmostEqual(env.agents[0].memories[61].reward, 18.2, 0)
    #     self.assertAlmostEqual(env.agents[0].memories[62].reward, 27.5, 0)
    #     # env.update_memory_rewards()
    #     exportData = ExportData(learningMethod='Monte Carlo TODO', learningEpochs=0, nets=env.global_memories,
    #                             netName='net11', densityName='test_no_5')
    #     exportData.saveToJson()
    #
    # #
    # # #
    ...
    def test_no_7_pass_action_0_all_time(self):
        # TESTUJEMY: caly czas powinna byc aktualna faza = [0]
        max_time = 90
        agents = get_SmartAgents()
        for agent in agents:
        ...
        for t in range(max_time):
            # if t >= 3:
            #     self.assertEqual([agent.actual_phase for agent in agents], [0])
            actions = get_seq_actions(t)
            env.step(actions)
        exportData = ExportData(learningMethod='Monte Carlo TODO', learningEpochs=0, nets=env.global_memories,
                                netName='net11',
                                densityName='test_no_7')
        exportData.saveToJson()
