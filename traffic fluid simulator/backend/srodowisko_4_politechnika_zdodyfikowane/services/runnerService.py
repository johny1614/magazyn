...
def epoch_greedy(env) -> Env:
    ...
    for t in range(Globals().vp.max_time_greedy):
        ...
        env.step(actions)
        for agent in env.agents:
            for i in range(len(agent.phase_absences)):
                agent.phase_absences[i] += 1
            if agent.actual_phase != yellow:
                agent.phase_absences[agent.actual_phase] = 0
    return env.u
