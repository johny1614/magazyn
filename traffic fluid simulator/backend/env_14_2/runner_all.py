...
def plot_rewards():
    plt.plot(Globals().global_rewards_mem, color='red', label='0')
    print('plooooooooooooooooooooooooot')
    # plt.plot([rew[0][1] for rew in Globals().global_rewards_mem], color='green', label='1')
    plt.legend()
...
def draw_predictions(no):
        ...
        for den1 in range(25):
            for den2 in range(25):
                for den3 in range(25):
                    to_predict.append([den0*multiplier, den1*multiplier, den2*multiplier, den3*multiplier, 1,0,0,0])  # na razie dla fazy 0
    ...
    for i in range(len(predictions)):
        ...
        best_action_predicted = np.argmax(pred[0:2])
    ...
    fig, ax = plt.subplots()
    for den1_ in range(25):
        for den3_ in range(25):
            den1=multiplier*den1_
            den3=multiplier*den3_
...
for run in runs:
    ...
    timeToLearn = 500000
    startTime = timer()
    generate_random_epochs(learntAgents=False, epochs=range(Globals().vp().first_epochs_range),u=env_settings.double_random_u())  # bierze nowych agentow i tu jest 'is'
