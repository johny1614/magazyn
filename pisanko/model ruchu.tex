...
\]
\begin{definicja}
Niech dana będzie macierz $A$ o wymiarach $nxm$ i wektor $B=[b_1,...,b_m]$. Mnożeniem macierzy przez wektor (*) nazywana jest następująca operacja:
\[
\begin{bmatrix}
a_{11} & ... & a_{1m}\\
... & ... & ...\\
...
\end{bmatrix}
\]
Powyższa operacja jest jednoznaczna z następującym mnożeniem macierzy:
\[
\begin{bmatrix}
a_{11} & ... & a_{1m}\\
... & ... & ...\\
...
Macierz $S(k)$ nazywana będzie \textbf{macierzą źródła wewnętrznego}. Dotyczy ona ruchu wpływającego do poszczególnych dróg w momencie $k+1$ ze wszystkich skrzyżowań układu. Jest to macierz rzadka, istotna obliczeniowo jest jedynie pierwsza kolumna, a pozostałe wartości są zerowe by osiągnąć wymiar macierzy pozwalający na dodawanie z $\overrightarrow{P(k)}$. Pierwsza kolumna jest zdefiniowana w oparciu o ostatnią kolumnę macierzy $P(k)$ i jest równa:
...
\begin{tcolorbox}
\textbf{Macierz źródła wewnętrznego z uwzględnieniem sygnalizacji świetlnej} jest przedstawiona wzorem:
\[S(k,0)=M(k) \cdot P(k,l) \addtag \label{eq:S} \]
Oprócz pierwszej kolumny macierz $S$ posiada jedynie wartości zerowe.\\
Wprowadzony zostanie ciąg $W(k)$ pomocniczych \textbf{macierzy oczekujących pojazdów}. Aby sformułować ten ciąg macierzy należy najpierw zdefiniować macierz pojazdów, które przejechały przez skrzyżowanie $L(k)$. Jest to kolejna macierz rzadka tutaj jedynie z wartościami dodatnimi w ostatniej kolumnie zdefiniowanej jako:
\[M(k) * P(k,l)^T\]
Podobnie $W(k)$ jest macierzą w której jedyne inne od zera wartości są w ostatniej kolumnie wedle poniższego wzoru.
\[
W(k)=P(k,l)-W(k)
\]

Finalny wzór na rozwój gęstości to:
...
Dotychczasowe rozważania zakładały, że do sieci dróg nie napływają nowe pojazdy. Oczywistą konsekwencją tego założenia jest brak pojazdów na drogach od pewnego momentu. Sieć dróg jest układem zamkniętym, zatem dla dróg początkowych należy ustalić pewne źródła ruchu. Wprowadzony jest ciąg \textbf{macierzy źródeł zewnętrznych} $O(k)$ o wymiarach takich samych jak macierz stanu. Macierz źródeł zewnętrznych to macierz rzadka - jej pierwsza kolumna przedstawia ilość pojazdów, które wpływają do dróg odpowiadających indeksowi wierszy. Dla rozważanego skrzyżowania przykładowa macierz źródła $O(k)$ jest następująca:
...
\[P(k+1)=\overrightarrow{P(k)}+S(k)+W(k)+O(k)\]
Ogólny wzór macierzy źródła jest następujący:
\[
O(k)=\begin{bmatrix}
O_1 & 0 &...& 0 \\
O_2 & 0 &...& 0 \\
...
\end{bmatrix}
\]
gdzie wartości w pierwszej kolumnie i m-tym wierszu określają ilość pojazdów wpływających do drogi $e_m$ w chwili $k$.
\end{tcolorbox}
...
k & M(k)& P(k) & $\overrightarrow{P(k)}$ & S(k,0) & W(k,l) & O(k,0) \rule{0pt}{18pt} \\
\hline
0 & \MZero & \PZero & \PmovedZero & \SZero & \WZero & \OZero   \rule{0pt}{35pt} \\  
1 & \MI & \PI & \PmovedI & \SI & \WI & \OI   \rule{0pt}{35pt} \\  
2 & \MII & \PII & \PmovedII  & \SII  & \WII  & \OII  \rule{0pt}{35pt} \\
3 &  & \PIII & & & & \rule{0pt}{35pt} \\
\hline
\end{tabular}
