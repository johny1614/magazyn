...
\end{definicja}
\begin{definicja}
Niech dana będzie macierz stanowa o wymiarach $n \times m$. Macierz $S^\theta$ jest macierzą początkową wektora kolumnowego $S$ o wymiarach $n \times 1$.
\[
...
\end{definicja}
\begin{definicja}
Macierzą diagonalną wektora $S$ nazywana jest następująca macierz:
\[
S^{\textbackslash}=
\begin{bmatrix}
s_1 & ... & 0 \\
...
Macierz $S(k)^0$ będzie nazywana \textbf{macierzą źródła skrzyżowań}. Jest to macierzą początkową wektora $S(k)$ wedle definicji (\ref{eq:S^0}).
...
\[P(k+1)=\overrightarrow{P(k)} + S(k)^{\theta} + W(k)^{\theta} \addtag \label{eq:P_sygnalizacja}\]
\end{tcolorbox}
\newpage 
\subsection{Zastosowanie wzoru}
