...
\end{figure}
Przedstawiony w pracy pomysł zakłada, że źródło $b_{in}$ i ujście $b_{out}$ ruchu znajdują się w odległości $\Delta x$ odpowiednio od punktu $b_0$ i $b_l$ (patrz Rysunek \ref{fig:odcinki-zrodlo-ujscie}). \\Gęstość w źródle oznaczamy jako $p^k_{in}$. Rozwój gęstości w początkowym punkcie drogi $b_0$ jest ustalony analogicznym wzorem do (\ref{eq:final_normal_development}):
...
\begin{figure}[H]
 ...
 \label{fig:skrz_1}
\end{figure}
Skrzyżowanie $v$ posiada dwie drogi wlotowe $e_1,e_2$ oraz jedną wylotową $e_3$. Źródła ruchu dla tej sieci dróg znajdują się na początku dróg $e_1,e_2$ (punkty 1 i 2 na Rysunku \ref{fig:skrz_1}). Oznaczmy $_1p_{out}^k$,$_2p_{out}^k$ jako ujścia dróg $e_1$ i $e_2$ do skrzyżowania $v$.
%Początkowo założone będzie, że nie ma jeszcze żadnych pojazdów na drodze $e_3$. 
Skrzyżowanie posiada dwie fazy świateł:\\
$M^{(1)}$ - jazda w lewo z drogi $e_1$\\
$M^{(2)}$ - jazda w prawo z drogi $e_2$ \\
Przy uwzględnieniu sygnalizacji możemy opisać gęstość źródła drogi $e_3$ jako:
\begin{numcases}{_3p_{in}^{k+1}=}
   _1p_{out}^k & \text{gdy } $M^{(1)}$ jest aktywną fazą\\
   _2p_{out}^k & \text{gdy } $M^{(2)}$ jest aktywną fazą 
\end{numcases}
Należy podkreślić, że ujścia dróg $e_1,e_2$ i źródło drogi $e_3$ są fizycznie w tym samym punkcie oznaczonym jako $v$. Szczegółowa dane pozwalające na przeprowadzenie symulacji są następujące: \\
$_1p_{in}^k=0.2$ \\
$_2p_{in}^k=0.1$ \\
