...
Niech $e$ będzie odcinkiem rozważanej drogi. Celem jest określenie ciągu $b_n$ wybranych punktów drogi $e$. Niech dana będzie liczba naturalna $l$ określająca ilość wybranych punktów z drogi. Wtedy odległość między sąsiednimi punktami to $\Delta x=\frac{L_e}{l}$. 
Ciąg $b_n$ jest zdefiniowany następująco: 
\[b_n=n\cdot \Delta x \quad \text{dla }n=0,1,...,l \addtag \label{eq:ciag_bazowy} \]
Powyższy ciąg (\ref{eq:ciag_bazowy}) będzie nazywany siatką bazową. Do celów czysto obliczeniowych przedstawionych w rozdziale \ref{sec:dyskretyzacja_ruchu} zostaje utworzona również siatka pomocnicza:
...
Od tego momentu rozważamy jedynie punkty niebrzegowe zarówno ciągu pomocniczego jak i bazowego - czyli z wyjątkiem $b_0,b_l,\widetilde{b_0},\widetilde{b_{l+1}}$.
...
\]
Powyższe wzory dotyczą $n=1,2,...,l-1$. 
%Dla krańcowych punktów dyskretyzacja prowadzi do wartości:
%\[\widetilde{p^k_0}=p^k_0\]
%\[\widetilde{p^k_l}=p^k_l\]
 Ustalona zatem będzie ewolucja gęstości dla pomocniczego ciągu $\widetilde{b_n}$. Jest ona opisana analogicznie do (\ref{eq:normal_center_development}):
...
\[ p_n^{k+1} =\frac{1}{2}\widetilde{p^{k}_n}-\frac{\Delta t}{2\Delta x}(q^k_n-q^k_{n-1})+\frac{1}{2}\widetilde{p^{k}_{n+1}}-\frac{\Delta t}{2\Delta x}(q^k_{n+1}-q^k_{n})  \addtag \label{eq:final_normal_development}.\]
...
\[ p_n^{k+1} =\frac{1}{2}p_n^k+\frac{1}{4}p_{n+1}^k+\frac{1}{4}p_{n-1}^k  +\frac{\Delta t}{2\Delta x}(q^k_{n-1}-q^k_{n+1})  \addtag .\]


Powyższy wzór kończy rozważania odnośnie punktów wewnętrznych drogi.
Przypadek punktów brzegowych różni się diametralnie od punktów wewnętrznych ze względu na to iż trzeba w nich ująć aspekty wprowadzenia nowych pojazdów do ruchu oraz opuszczenia drogi. Podstawowe równanie modelu makroskopowego (\ref{main_diff_eq}) nie uwzględnia ani źródła, ani też ujścia ruchu, co pozwala na zastosowanie własnego rozwiązania w tym zakresie.
Wartość $\widetilde{p_0^k}$ jest źródłem ruchu. Początkowo ustalmy, że jest ona z góry dana i stała na przedziale $(\widetilde{b_0},b_0)$. Wtedy na podstawie wzoru \ref{eq:calki-LWR} otrzymujemy:
\[\widetilde{p_0^{k+1}} \frac{\Delta x}{2} + \int\limits_{b_{0}}^{\widetilde{b_{1}}} {p(x,t_{k+1})dx-\widetilde{p_0^{k}} \frac{\Delta x}{2}-\int\limits_{b_{0}}^{\widetilde{b_{1}}}p(x,t_{k})dx} +\int\limits_{t_k}^{t_{k+1}} q(\widetilde{b_{1}},t)-q(\widetilde{b_{0}},t))dt=0 \addtag \]
...
\begin{numcases}{\gamma^{k}_{out}=}
   q(\widetilde{p^k_{l+1}}) & \text{dla } $\widetilde{p^k_{l+1}}\in[0,p^{*}]$\\
   q(p^*) & \text{dla } $\widetilde{p^k_{l+1}}\in(p^{*},p^{max}]$ 
\end{numcases}
Rozwój gęstości ruchu w skrajnych punktach ciągu $\widetilde{b_n}$ przedstawia się następująco:
\[\widetilde{p_0^{t+1}}=\widetilde{p_0^{t}}-\frac{\Delta t}{\frac{1}{2}\Delta x}(f(p_0^t)-\gamma_{in}^t) \addtag \label{eq:stagered_start_development} \]
\[\widetilde{p_{l+1}^{t+1}}=\widetilde{p_{l+1}^{t}}-\frac{\Delta t}{\frac{1}{2}\Delta x}(f(p_0^t)-\gamma_{out}^t) \addtag \label{eq:stagered_end_development}  \]
\\
